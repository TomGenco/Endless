...
this.finished=!1;var a=this;setTimeout(function(){a.finished=!0},a.delay+a.duration);k=Date.now()};this.getVal=function(){var a;if(this.finished)return this.endVal;if(this.started){m=Date.now()-this.delay-k;if(0>m)return this.startVal;switch(this.motionType){default:console.error("Should be unreachable");case "linear":return a=m/this.duration*n+this.startVal,a>b?b:a;case "logistic":return 1/(1+Math.pow(Math.E,-15*(m/this.duration-.5)))*n+this.startVal}}else return this.startVal}};(function(){var a=
document.getElementsByTagName("h1")[0];a.removeAttribute("style");for(var b in Settings.DefaultEntries)Settings.Entries[b]=Settings.DefaultEntries[b];window.localStorage?Settings.loadFromStorage():console.error("This browser doesn't support Web Storage, so settings and statistics cannot be saved to browser.");setTimeout(function(){D();G();a.setAttribute("style","display:none;")},300)})()}window.onload=Endless;
